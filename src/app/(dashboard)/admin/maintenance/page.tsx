  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500">{total} total requests</p>
      </div>

      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          <input type="text" placeholder="Search requests..." className="w-full rounded-md border border-slate-200 py-2 pl-10 pr-4 text-sm focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} />
        </div>
        ...
        <select className="rounded-md border border-slate-200 px-3 py-2 text-sm" value={priorityFilter} onChange={(e) => { setPriorityFilter(e.target.value); setPage(1) }}>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Property / Unit</th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Requester</th>
          ...
          <tbody className="divide-y divide-slate-200">
            {isLoading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i}><td colSpan={6} className="px-6 py-4"><div className="h-6 animate-pulse rounded bg-slate-200" /></td></tr>
              ))
            ) : requests.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-12 text-center"><Wrench className="mx-auto h-12 w-12 text-slate-300" /><p className="mt-2 text-sm text-slate-500">No maintenance requests</p></td></tr>
            ) : (
              requests.map((req: Record<string, unknown>) => (
                <tr key={req.id as string} className="cursor-pointer hover:bg-violet-50/30" onClick={() => router.push(`/admin/maintenance/${req.id}`)}>
                  <td className="px-6 py-4">
                    <div className="font-medium">{req.title as string}</div>
                    <div className="text-sm text-slate-500">{req.category as string}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-sm text-slate-500">{(req.property as { name: string })?.name} / Unit {(req.unit as { number: string })?.number}</td>
                  <td className="px-6 py-4 text-sm text-slate-500">{(req.requester as { name: string })?.name}</td>
                  <td className="px-6 py-4 text-sm text-slate-500">{formatDate(req.createdAt as string)}</td>
                </tr>
              ))
            )}
          </tbody>
